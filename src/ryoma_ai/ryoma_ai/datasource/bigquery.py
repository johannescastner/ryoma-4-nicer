...
from ryoma_ai.datasource.metadata import (  # ensure these are correct
    Catalog,
    Column,
    Schema,
    Table,
...
class BigQueryInspector(Inspector):
            ...
            if hasattr(self.engine, "raw_connection"):
                ...
                if hasattr(conn, "credentials") and hasattr(conn, "project"):
                    self.bq_client = bigquery.Client(
                        credentials=conn.credentials, project=conn.project
                    )
    ...
    def get_columns(self, table_name, schema=None, **kw):
        ...
        if (
            cache_key in self._table_info_cache
            and "columns" in self._table_info_cache[cache_key]
        ):
            return self._table_info_cache[cache_key]["columns"]

        try:
            table_ref = self.bq_client.get_table(
...
class BigQueryDataSource(SqlDataSource):
    def __init__(
        ...
        *,
        ...
        self.dataplex_metadata_lookup = (
            self._build_metadata_lookup() if metadata else {}
        )
    ...
    def get_catalog(self, catalog: Optional[str] = None) -> Catalog:
        return Catalog(
            ...
            schemas=[
                Schema(
                    schema_name=(
                        table.schema if hasattr(table, "schema") else "default_schema"
                    ),
                    tables=[
                        Table(
    ...
    def query(self, sql: str):  # -> pandas.DataFrame
        ...
        if (
            "INFORMATION_SCHEMA" in sql.upper()
            or "LIST DATASETS" in sql.upper()
            or "SHOW DATASETS" in sql.upper()
            or "SHOW TABLES" in sql.upper()
        ):
            try:
    ...
    def _qualify(self, table_name):
        if table_name in self.metadata_lookup:
            ...
            raise ValueError(
                f"Cannot qualify table '{table_name}' without dataset_id or metadata."
            )
